...
const AggregatedAnalysisOutput = ({ analysis, timestamp, agentsCount, marketUrl }: AggregatedAnalysisOutputProps) => {
  ...
  const allLines = [
    ...
    ...(analysis.agentConsensus?.dissenting && analysis.agentConsensus.dissenting.length > 0 
      ...
      : []),
    { type: "divider", content: "─".repeat(50) },
    { type: "price", content: `MARKET PROBABILITY: ${analysis.marketProbability != null ? analysis.marketProbability.toFixed(1) : 'N/A'}%` },
    { type: "price", content: `ESTIMATED ACTUAL: ${analysis.estimatedActualProbability != null ? analysis.estimatedActualProbability.toFixed(1) : 'N/A'}%` },
    { type: "edge", content: `ALPHA OPPORTUNITY: ${analysis.alphaOpportunity != null ? ((analysis.alphaOpportunity > 0 ? "+" : "") + analysis.alphaOpportunity.toFixed(1)) : 'N/A'}%` },
